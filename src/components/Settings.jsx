...
export default function Settings({ onBack }) {
    // State to hold all our settings values with local storage persistence
    const [settings, setSettings] = useState(() => {
        const saved = localStorage.getItem('energySettings');
    ...
    const handleReset = () => {
        if (window.confirm('Are you sure you want to reset all settings to default?')) {
            ...
            localStorage.setItem('energySettings', JSON.stringify(defaultSettings));
    ...
    const addSlab = () => {
        const lastSlab = settings.slabs[settings.slabs.length - 1];
        const newFrom = lastSlab.to === '∞' ? 1 : (typeof lastSlab.to === 'number' ? lastSlab.to + 1 : 1);
        setSettings(prev => ({
            ...prev,
            slabs: [...prev.slabs, { from: newFrom, to: '∞', rate: 35 }]
        }));
    };
        ...
        <div className="min-h-screen bg-darkbg text-gray-300 pb-20">
                        ...
                        <div className="mt-4 bg-[#0a0a0a] p-4 rounded-lg border border-gray-800">
                            ...
                            {settings.slabs.map((slab, i) => (
                                <div key={i} className="flex items-end gap-2 mb-3">
                                    ...
                                    <button onClick={() => deleteSlab(i)} className="p-1.5 text-gray-600 hover:text-red-500 transition-colors"><svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                                </div>
                            ))}
                            <button onClick={addSlab} className="w-full py-2 border border-dashed border-gray-700 rounded text-xs text-gray-400 hover:text-white hover:border-gray-500 transition-colors flex items-center justify-center gap-1 mt-2">
            ...
            <div className="fixed bottom-0 left-0 right-0 bg-[#0a0a0a] border-t border-gray-800 p-4 z-50">
                <div className="max-w-3xl mx-auto flex justify-between gap-4 items-center">
                    <button onClick={handleSave} className="flex-1 bg-brandBlue hover:bg-blue-500 text-white font-medium py-3 rounded-lg flex items-center justify-center gap-2 transition-colors">
                    ...
                    <button onClick={handleReset} className="px-6 py-3 border border-gray-700 text-gray-400 hover:text-white hover:border-gray-500 rounded-lg flex items-center justify-center gap-2 transition-colors">
